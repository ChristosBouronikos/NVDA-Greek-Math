...
* **Voice options and downloads**: categorized Voices & Tools settings providing direct download links for high-quality Greek voices in Windows OneCore and Nuance Vocalizer Expressive (Melina / Nikos). (Experimental neural voice synthesizer is deferred from active release, preserved for future releases).
...
This version (2.5.6) ships on the stable channel by explicit maintainer decision. Review gates for semantic terminology remain open, tracked in `RELEASE_GATES.md`, `TERMINOLOGY_REVIEW.md`, and `TTS_ENGINE_RESEARCH.md` in the source repository.
